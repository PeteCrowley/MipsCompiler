structure PrintExpty:
sig
  val print: TextIO.outstream * Semant.expty -> unit
end =
struct
  structure A = Absyn
    ...
    let
      ...
      fun typename (Types.RECORD (r, _)) = "RECORD"
        ...
        | typename (Types.ARRAY (ty, _)) =
            "ARRAY[" ^ typename ty ^ "]"
        | typename (Types.ARROW (tylist, ret)) =
            "ARROW[" ^ typename ret ^ "]"
        | typename Types.UNIT = "UNIT"
        | typename Types.BOTTOM = "BOTTOM"

      fun do_expty (expty: Semant.expty) =
        sayln (typename (#ty expty))
      ...
      fun dolist d f [a] =
            (sayln ""; f (a, d + 1))
        | dolist d f (a :: r) =
            (sayln ""; f (a, d + 1); say ","; dolist d f r)
        | dolist d f nil = ()
